...
function App() {
  ...
  return <main>
    ...
    {showLOR && !hasSearch && <section className="content container">
      ...
      <div className="lor-collection-layout"><aside className="sequence-sidebar"><p className="section-label">Sequence index</p><nav aria-label={`${route.lOR} sequence index`}>{routePages.map((page) => <a key={pageKey(page)} href={`#${pageAnchor(page)}`}><strong>SEQ {page.sequence}</strong><span>{page.location}</span></a>)}</nav></aside><div className="page-results">{routePages.map((page, index) => details.get(pageKey(page)) && <PageDetail key={pageKey(page)} page={details.get(pageKey(page))} previous={routePages[index - 1]} next={routePages[index + 1]} connectionTargets={connectionTargets} connectionLORs={connectionLORs} inCollection />)}</div></div>
    </section>}
...
function PageDetail({ page, previous, next, connectionTargets, connectionLORs, inCollection = false }) {
  ...
  return <article className="page-detail" id={pageAnchor(page)}>
    ...
    <section className="connections-section"><DetailList title="Connections" items={page.connections} connectionTargets={connectionTargets} connectionLORs={connectionLORs} region={page.region} /></section>
    <details className="textual-details"><summary><span><span className="section-label">Textual representation</span><strong>Operational details and page transcription</strong></span><span className="textual-details-toggle">Show</span></summary><div className="detail-grid">{page.locations && <DetailList title="Locations & mileages" items={page.locations} />}<DetailList title="Signalling & communications" items={page.signalling} /><DetailList title="Running lines & speed restrictions" items={page.speeds} /><section className="detail-card equipment"><h3>Equipment</h3><p>{page.equipment}</p></section></div><section className="transcription"><div><p className="section-label">Textual representation</p><h3>Page transcription</h3></div><p>{page.transcription}</p></section></details>
  </article>;
}
